...
const MustWatchMoviesPage = () => {
  ...
  // build a lookup of all cached movies by id (safely)
  const byId = useMemo(() => {
    const map = new Map();
    ...
    return map;
  }, [buckets.map((b) => (b ? 1 : 0)).join(",")]); // cheap dep; avoids heavy refs

  // pick the ones marked as mustWatch (skip those not in cache)
